...
while True:
    ...
    if want == "delete unimportant files":
        ...
        for f in files:
            if not f.endswith('delete.py') and not f.endswith('commands.py') and not f.endswith('config.py') and not f.endswith('main.py') and not f.endswith('db.sql') and not f.endswith('permissions.py'):
                os.remove(f)
